...
const Carousel = ({ imageUrls }) => {
    ...
    useEffect(() => {
        const interval = setInterval(nextSlide, 2000); // Change interval time here (in milliseconds)
            ...
            <div className="relative h-56 overflow-hidden rounded-lg md:h-96">
                ...
                {imageUrls.map((url, index) => (
                    ...
                    >
                        <img
                            src={url}
                            className="absolute block w-auto h-auto -translate-x-1/2 -translate-y-1/2 top-1/2 left-1/2"
                            alt={`Slide ${index + 1}`}
                        />
                        </div>
